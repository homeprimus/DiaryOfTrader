...
namespace DiaryOfTrader.Core.Entity.Economic
{
  public class EconomicParser
  {
    ...
    public static List<EventCalendar> MakeEventCalendar(List<EconomicSchedule> events)
    {
      return events
        .Join(
          ...
          while (trMatch.Success && !cancellationToken.IsCancellationRequested)
          {
            ...
            if (!trValue.Contains(Resources.EconomicWeekEnd + "</span>"))
            {
              ...
              else if (trValue.Contains("class=\"noResults center\""))
              {
                ...
                {
                  var ev = new EconomicSchedule { Time = startDate, Description = noResultMath.Result("${value}") };
                  result.Add(ev);
                }
              }
              else
            ...
            if (!string.IsNullOrEmpty(html))
            {
              ...
              if (i > -1)
              {
                ...
                var result = new EconomicEvent
                {
                  ...
                  Country =
                    GetValue(@"<span>" + Resources.EconomicCountry + @"<\/span>\n?<span><i title=""(?<value>.*?)""", s),
                  Currency =
                    GetValue(@"<span>" + Resources.EconomicCurency + @"<\/span>\n?<span>(?<value>.*?)<\/span>", s),
                  SourceRef = GetValue(
                    @"<span>" + Resources.EconomicSource + @"<\/span>\n?<span><a href=""(?<value>.*?)""", s)
                };
